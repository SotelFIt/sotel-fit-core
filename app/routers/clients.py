...
def bootstrap_admin(payload: CreateClientRequest, db: Session = Depends(get_db)):
    """Cria admin inicial (sem auth). Só funciona se não houver admin."""
    existing_admin = db.execute(
        text("SELECT id FROM clients WHERE objective = 'admin' LIMIT 1")
    ).fetchone()
    ...
    result = db.execute(
        text("""
            INSERT INTO clients (name, email, phone, objective, status, created_at)
            VALUES (:name, :email, :phone, 'admin', 'active', NOW())
